...
int_var,str_var,bool_var, def_func, print_stat, arg_list, def_return = \
    g.non_terminals('<int-var> <str-var> <bool-var> <def-func> <print-stat> <arg-list> <def-return>')          # Definiciones
                    # Expresiones y definiciones Build in
...
patientx, name, sex, age, addx, removex, lenx, find, breast_cancer, ovarian_cancer, pancreatic_cancer \
...
stat %= patient_expr+semi,          lambda h, s: s[1]                     
...
for_decl %= forx + opar + idx + equal + num + semi + expr + semi + idx + minus + minus + cpar + okey + stat_list + ckey, \
            lambda h, s: ForNode(s[3], s[5], s[7], s[9], s[10], s[11], s[14])
...
def_attr_add    %= idx + dot + addx  + opar  + expr + cpar, lambda h,s: AddNode(s[1], s[5])
def_attr_len    %= idx + dot + lenx  + opar  + cpar, lambda h,s: LenNode(s[1])
def_attr_remove %= idx + dot + removex  + opar  + expr + cpar, lambda h,s: RemoveNode(s[1], s[5])

patient_expr          %= patientx + opar  + stringx + comma + stringx +comma+ num + cpar, lambda h,s: PatientNode(s[3],s[5],s[7])  
def_find              %= find     + opar  + idx     + comma + stringx + cpar, lambda h,s: FindNode(s[3], s[5])
def_breast_cancer     %= breast_cancer    + opar  + idx + cpar, lambda h,s: BreastCancerNode(s[3])
def_ovarian_cancer    %= ovarian_cancer   + opar  + idx + cpar, lambda h,s: OvarianCancerNode(s[3])
def_pancreatic_cancer %= pancreatic_cancer+ opar  + idx + cpar, lambda h,s: PancreaticCancerNode(s[3])

def_func %= funcx + idx + opar + arg_list + cpar + okey + stat_list + ckey, \
...
#expr %= def_search_good_strategy, lambda h, s: s[1], None
